...
for (var i = 0; i < array_length(levels); ++i) {
	...
	if(global.selected == i){//display best time
		...
		if(unlocked_levels <= i){
			...
			if(star[i] == 0) {
				//IF STAR = 0?
				draw_sprite_ext(spr_icon_star,0,view_w/2+(i*offset)-(global.selected*offset)-34,view_h-21+pandown-5,1,1,0,c_dkgray,1);
			} else {
				//IF STAR = 1?
				draw_sprite_ext(spr_icon_star,0,view_w/2+(i*offset)-(global.selected*offset)-34,view_h-21+pandown-5,1,1,sinwave(-6,6,2,0),c_white,1);	
			}
