...
function text_box() {
	...
	var output = instance_create_layer(0,0,"Instances",obj_dialogue);
	output.font[@ 0] = ""
	...
	//Font indicator
		...
		else output.font[@ i] = "f_action_man";
	...
	//Portrait indicator
		if string_pos("#port_",input[@ i]) != 0 {
			...
			output.port[@ i] = string_copy(input[@ i],port_start,port_end-1);
		}
		//If there's no indicator to be found, the textbox will re-use the previously chosen one.
		else if (i > 0) output.port[@ i] = output.port[i-1];
		//If there is no indicator to be found, and it's the first message, it'll default to this
		else output.port[@ i] = "port_none"
	...
	//Sound indicator. (The sound to play when letters are typed, will take sound from portrait by default)
			...
			else
			{
				...
				output.sound[@ i] = asset_get_index("vo_" + sound);
			}
		}
