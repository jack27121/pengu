...
//	if (place_meeting(x+hspd_, y, obj_wall) || out_cam_bounds(x+hspd_, y)) {
//		//moves the player one pixel closer until it's just next to the wall, and then stop moving
//		while(!place_meeting(x+sign(hspd_), y, obj_wall) && !out_cam_bounds(x+sign(hspd_), y)) {
//			x += sign(hspd);
//		}
//		vspd = vspd * frict; //friction
//		hspd = hspd * -bounciness;
//	} else x+= hspd;
//	
//	// Vertical collisions
//	if (place_meeting(x, y+vspd_, obj_wall) || out_cam_bounds(x, y+vspd_)) {
//		while(!place_meeting(x, y+sign(vspd_), obj_wall) && !out_cam_bounds(x, y+sign(vspd_))) {
//			y += sign(vspd);
//		}
//		hspd = hspd * frict;
